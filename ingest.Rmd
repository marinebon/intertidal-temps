...
for (i in 1:length(levels(d$site))) { # for each site i
  ...
  write_csv(
    d_site_avg,
    glue("{getwd()}/data_smoothed/{site}.csv"))
...
### actually read in data and metadata
```{r warning=FALSE}
...
for (i in 1:length(d_filtered)) {
  ...
  d_filtered[[i]] <- d_filtered[[i]] %>% 
    bind_rows %>% group_by(zone)
  d_dailyq <- dailyQuantiles_gdata(d_filtered[[i]])
...
for (i in 1:length(levels(d$site))) { # for each site i
  ...
  if (length(c(levels(d_site$zone == 1)))) {
    d_site <- d_site %>% 
      filter(metric %in% c("temp_c_avg", "temp_c_min", "temp_c_max"))
  }
  else if (length(levels(d_site$zone)) > 1) {
    d_site <- d_site %>% 
      filter(metric == "temp_c_avg") %>% 
      select(-site, -metric) %>% 
...
for (i in 1:nrow(m_info)) {
  m_meta <- m_info %>% 
    filter(id == m_info$id[i]) 
  yaml_m_meta <- as.yaml(list(m_meta))
  write_yaml(
    yaml_m_meta,
    glue("{getwd()}/data_smoothed/{m_info$id[i]}_meta.yml"),
    fileEncoding = "UTF-8")
}
```
...
write_csv(psites_smoothed, glue("{getwd()}/data_smoothed/psites_smoothed.csv"))

# read metadata back in
p2p_info <- read_csv(here::here("data_smoothed/psites_smoothed.csv"))

# write yaml metadata files
for (i in 1:nrow(p2p_info)) {
  p2p_meta <- p2p_info %>% 
    filter(id == p2p_info$id[i]) 
  yaml_p2p_meta <- as.yaml(list(p2p_meta))
  write_yaml(
    yaml_p2p_meta,
    glue("{getwd()}/data_smoothed/{p2p_info$id[i]}_meta.yml"),
    fileEncoding = "UTF-8")
}
```
