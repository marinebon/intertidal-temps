...
d <- read_tsv(tsv) %>% 
  mutate(
    time = parse_date_time(d$Time_GMT, "m/d/y H:M")) %>% # parse through datetimes, store as time col.
...
# convert to xts  
x_movingwindow <- xts(select(d_movingwindow_mean, -time), order.by=d_movingwindow_mean$time) 
...
x_movingwindow_orig <- xts(select(d_movingwindow_orig, -time), order.by=d_movingwindow_orig$time) 
View(x_movingwindow_orig)
