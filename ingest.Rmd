...
# 2021-02-24 POSSIBLE TO-DO's:

- Finish merging these 4 sites that are reasonably close to a P2P site

- Start exploring P2P Robolimpet data here:
  - Ben's path of an example: `/Volumes/GoogleDrive/My Drive/projects/mbon-p2p/Robolimpets/Data/Argentina/Gregorio/data/0444 B300 C717 08-20190301 130640.csv`

```{r}
dir_p2p_robo <-
  case_when(
    user == "bbest"       ~ "/Volumes/GoogleDrive/My Drive/projects/mbon-p2p/Robolimpets",
    # user == "cdobbelaere" ~ "/Users/cdobbelaere/Documents/robomussels/Robolimpets")
...
# find where metadata ends (from github help):
...
read_metadata <- function(path) {
  ...
  if (find_skip(file = path, pattern = "^time,") != Inf) {
    metadata <- tibble(
      metadata = read_lines(
        path, n_max = (find_skip(file = path, pattern = "^time,") - 1))) %>% 
    ...
    metadata <- metadata %>% mutate_all(list(~gsub(",", "", .)))
    
  # if no metadata present:
  } else metadata <- read_csv(path)
...
# read all csv files and combine data & metadata
d <- tibble(
  path     = paths,
  file     = basename(path),
  data     = map(path, read_tempcsv),
  metadata = map(path, read_metadata))

# 7 has no metadata, just temp data
# 13 has a weird character that needs to be removed
```
...
```{r}
# join by nearest feature, 4 sites that are nearby
...
max(sites_joined$dist_km) # 2.656662
...
# get nearest points between msites and psites

# near_m2p <- st_nearest_points(d_msites_sf, d_psites_sf) %>% 
#   st_as_sf() %>% 
#   mutate(
#     dist_km = st_distance(x)) 
...
# Combine & smooth temp data from sites_joined
...
### split, smooth, & write csv's with temp data corresponding to sites in sites_joined (MARINe & p2p)
...
smoothed_files <- tibble(
  path = list.files(here::here("data_smoothed"), full.names = T),
  file = basename(path),
...
dygraph(eval(parse(text = paste0("x_", smoothed_files$site[2]))), main = "Daily Temperature") %>%
  dyHighlight(
...
# dygraph function
dyPlot <- function(x_data, cols) {
  dygraph <- dygraph(eval(parse(text = x_data)), main = "Daily Temperature") %>%
    dyHighlight(
      highlightCircleSize = 5, 
      highlightSeriesBackgroundAlpha = 0.2,
      hideOnMouseOut = TRUE) %>%
      dyOptions(
        colors = as.character(cols[names(x_data)])) %>% 
    dyOptions(
      fillGraph = FALSE, fillAlpha = 0.4) %>%
    dyRangeSelector()
  return(dygraph)
...
for (i in 1:length(smoothed_files$site)) {
  x_data <- paste0("x_", smoothed_files$site[i])
  dygraph <- dyPlot(x_data = x_data, cols = cols)
}
dygraph
...
# not in situ data (SST satellite data)
